    ...
    <div className="container-fluid">
      <div className="search-box text-center">
        <div className="input-wrapper">
          <input
            placeholder="What breed you looking for?"
            style={{ textTransform: "lowercase" }}
            onChange={event => {
              const value = event.target.value;
              console.log(value);
              const filtered = data.filter(dog =>
                dog.name.toLowerCase().includes(value)
              );
      ...
      <ul className="row justify-content-center">
        {filtered.map(dog => (
          <div className="col-md-4 col-sm-6 card my-3 py-3" key={dog.name}>
            <Fade delay={200}>
              <li className="cards text-center" key={dog.id}>
                ...
                {dog.id !== 196 && dog.id !== 197 && dog.id !== 261 ? (
                  <p className="m-2">
                    I am{" "}
                    <span className="text-lowercase">{dog.temperament}</span>
                  </p>
                ) : (
                  <div>I am loving dog</div>
                )}
              </li>
              <div className="text-center">
                <div
                  className="text-center"
                  onClick={() => addDogToFavorites(dog)}
                >
